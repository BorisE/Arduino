...
SPD		- set speed
SPC		- current speed
ACL		- set accelaration
CLN		- current cylce lenght
MVE		- currently moving (1|0)
DST		- destination position
OPN		- open sensor state (1|0)
CLS		- closed sensor state (1|0)
!r		- measured cycle len


OPEN	- открыть
CLOSE	- закрыть
STOP	- остановить
MOV:	- перейти на .. позицию
SET:	- установить текущую позицию

CLN:	- установить длину цикла (количество шагов для открытия/закрытия, учитывается знак!)
ACL:	- установить ускорение
SPD:	- установить базовую скорость
MND:	- установить размер мин. дистанции, в течении которой не проверяется нажата ли кнопка или нет. Т.е. двигатель в любом случае попытается сдвинуться на это количество шагов
