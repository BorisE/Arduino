void ProcessSerial()
{
  ...
  else if (cmdnamebuf=="#CLOSE")
  {
    //Start moving
    initMoving = true;
    setDirection(directionClose);
    Serial.println("CMD: close cover");
  }  
  
  else if (cmdnamebuf=="#MOVE")
  {
    //Start moving
    initMoving = true;
    stepperTargetPos = cmdvalbuf.toInt();
    Serial.println("CMD: start auto moving");
  }  
  ...
  {
    //Set speed
    setMoveStepperSpeed (cmdvalbuf.toInt() );
  }
  else if (cmdnamebuf=="#STP")
  {
    //Set steps per loop
